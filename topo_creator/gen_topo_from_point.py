...
class TopoFileGenerator:
    def __init__(self, data_services, box_sheeter_creator) -> None:
        # 仅初始化一次（使用类级别标记判断）
    ...
    def gen_topo_files(self, output_dir, timestamp):
        ...
        sro_boxes = self.sro_service.get_all_sro_order_by_code_asc()
    ...
    def init_workbook(self):
        ...
        for col, width in COLUMN_WIDTHS.items():
            ws_topo.column_dimensions[col].width = width

        return wb, ws_topo


    def generate_sro_topo_wb(self, output_path, sro):
        """生成拓扑Excel的主函数"""
        wb, ws_topo = self.init_workbook()
        current_row = 2  # 起始行

        sro_sheet_name = sro[BOX_CODE_FIELD_NAME]
        current_row = self.draw_point_and_resources(ws=ws_topo, current_row=current_row, box_data=sro, upper_cable_level=0,
                                               upper_section_value="N/A", sro_sheet_name=sro_sheet_name)
        sub_cable_amt = self.cable_service.get_all_1st_segments_start_with_box_order_by_code_asc(
    ...
    def draw_point_and_resources(self, ws, current_row, box_data, upper_cable_level, upper_section_value, sro_sheet_name):
        """判断点是否需要描绘分支线，至少两个子线缆时，初始化点的分支线的描绘开关、起止点行数"""
        (need_to_draw_box_vertical_branch_line,
         box_vertical_branch_line_start_row) = self.does_need_to_draw_box_vertical_branch_line(
            current_row, box_data)

        """==================BOX描绘开始=================="""
        """描绘BOX点"""
        current_row = self.draw_box_node_and_next_segment_in_same_section(ws, current_row, box_data, upper_cable_level,
                                                                     upper_section_value,
                                                                     need_to_draw_box_vertical_branch_line, sro_sheet_name)

        """查询以该BOX为起点, 但section!=box的section的所有第一段segments,"""
        first_segments_list = self.cable_service.get_all_1st_segments_start_with_box_order_by_code_asc(
        ...
        if first_segments_list is None or first_segments_list.empty:
            ...
            for _, _1st_segment in first_segments_list.iterrows():
                ...
                if need_to_draw_box_vertical_branch_line and sub_cable_idx == sub_cables_amt:
                    ...
                    self.draw_box_vertical_branch_line(ws=ws, upper_cable_level=upper_cable_level,
                ...
                (need_to_draw_cable_vertical_route_line,
                 cable_vertical_route_start_row) = self.does_need_to_draw_cable_vertical_right_line(current_row, _1st_segment)
                """开始描绘线缆第一个section横向部分(和自身描绘空间的右侧边框 bool参数)"""
                current_row = self.draw_1st_segment(ws, current_row, _1st_segment, upper_cable_level,
                                               need_to_draw_cable_vertical_route_line)
                ...
                """==================子线缆掏芯点开始=================="""
                """查询线缆上的掏芯点列表"""
                sub_boxes_on_complete_cable = self.get_sub_boxes_on_one_complete_cable_start_with_1st_section(_1st_segment, [])
                ...
                for box_on_cable in sub_boxes_on_complete_cable:
                    ...
                    if need_to_draw_cable_vertical_route_line and sub_box_idx == sub_boxes_amt:
                        ...
                        self.draw_cable_vertical_route_line(ws=ws, cable=_1st_segment, start_row=cable_vertical_route_start_row,
                                                       end_row=current_row)
                    """||||||||||继续调用递归函数描绘点||||||||||"""
                    current_row = self.draw_point_and_resources(ws=ws, current_row=current_row, box_data=box_on_cable,
    ...
    def get_sub_boxes_on_one_complete_cable_start_with_1st_section(self, segment, sub_boxes_list: list):
        if segment is not None and not segment.empty:
            section = segment[CABLE_SECTION_FIELD_NAME]
            box = self.box_service.get_box_by_code(segment[CABLE_EXTREMITY_FIELD_NAME])
            if not box.empty:
                sub_boxes_list.append(box)
                next_segment = self.cable_service.get_next_segment_by_origin_code(section, box[BOX_CODE_FIELD_NAME])
                return self.get_sub_boxes_on_one_complete_cable_start_with_1st_section(next_segment, sub_boxes_list)
            else:
    ...
    def draw_box_node_and_next_segment_in_same_section(self, ws_topo, start_row, box_data, upper_cable_level,
        ...
        sheet_name = self.box_sheeter_creator.create_box_sheet(ws_topo, col, box_data, sro_sheet_name)
        # 子sheet内容暂时留空（无需额外操作）
        current_row = self.draw_box_node(ws_topo, start_row, col, box_data, sheet_name, need_to_draw_box_vertical_branch_line)
        next_segment = self.cable_service.get_next_segment_by_origin_code(upper_section, box_data[BOX_CODE_FIELD_NAME])
        if next_segment is not None and not next_segment.empty:
            self.draw_vertical_segment(ws_topo, current_row, col, next_segment)
        return current_row


    def draw_box_node(self, ws, start_row, col, box_data, sheet_name, need_to_draw_box_vertical_branch_line):
    ...
    def draw_sro_node(self, ws, start_row, sro_data, need_to_draw_box_vertical_branch_line):
    ...
    def draw_vertical_segment(self, ws, start_row, col, segment):
        section = segment[CABLE_SECTION_FIELD_NAME]
        code = segment[CABLE_CODE_FIELD_NAME]
        type = segment[CABLE_TYPE_FIELD_NAME]
        length = segment[CABLE_LENGTH_FIELD_NAME]
        line_5_right_border = self.get_cell_right_boarder(ws, start_row + 5, col)
    ...
    def draw_closure_pbo_node(self, ws, start_row, box_data, upper_cable_level, need_to_draw_box_vertical_branch_line):
    ...
    def draw_cable(self, ws, start_row, cable_data, upper_cable_level, need_to_draw_cable_vertical_right_line):
    ...
    def draw_1st_segment(self, ws, start_row, section_data, upper_cable_level, need_to_draw_cable_vertical_right_line) -> int:
    ...
    def draw_section(self, ws, start_row, section_data, is_first_section, upper_cable_level,
    ...
    def draw_box_vertical_branch_line(self, ws, upper_cable_level, start_row, end_row):
    ...
    def draw_cable_vertical_route_line(self, ws, cable, start_row, end_row):
    ...
    def does_need_to_draw_box_vertical_branch_line(self, current_row, box_data):
        sub_cables_amt = self.cable_service.get_sub_cables_amt(box_data[BOX_CODE_FIELD_NAME])
        if sub_cables_amt >= 2:
    ...
    def does_need_to_draw_cable_vertical_right_line(self, current_row, _1st_segment_data):
        has_at_least_2_sections = self.cable_service.has_at_least_2_segments_on_cable(_1st_segment_data)
        if has_at_least_2_sections:
            return True, current_row + GROUP_ROWS
        else:
            return False, None

    def merge_cells(self, ws, start_row, end_row, col, value, border=None, font=None, align=None):
        """
        合并单元格并设置内容和样式
        :param ws: 工作表
        :param start_row: 起始行
    ...
    def get_cell_right_boarder(self, ws, row, col):
        cell = ws[f"{col}{row}"]
